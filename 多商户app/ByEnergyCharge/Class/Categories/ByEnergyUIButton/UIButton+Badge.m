...
{
    // 初始化，设定默认值
    self.badgeBGColor   = self.badgeBGColor?:[UIColor redColor];
    self.badgeTextColor = self.badgeTextColor?:[UIColor whiteColor];
    self.badgeFont      = self.badgeFont?:[UIFont systemFontOfSize:12.0];
    self.badgePadding   = self.badgePadding?:6;
    self.badgeMinSize   = self.badgeMinSize?:8;
    self.badgeOriginX   = self.badgeOriginX?:self.frame.size.width - self.badge.frame.size.width/2;
    self.badgeOriginY   = self.badgeOriginY?:-4;
...
-(void) setBadgeValue:(NSString *)badgeValue
{
    ...
    if (!badgeValue || [badgeValue isEqualToString:@""] || ([badgeValue isEqualToString:@"0"] && self.shouldHideBadgeAtZero)) {
        ...
        NSLog(@"11111-------%f,%f,%f,%f",self.badgeOriginX,self.badgeOriginY,self.badge.zm_width,self.badge.zm_height);

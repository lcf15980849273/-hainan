...
@interface BEnergyBaseViewController ()<UIGestureRecognizerDelegate>

@end

@implementation BEnergyBaseViewController
...
- (void)byEnergyNavItemWitnTitles:(NSArray *)titles
    ...
    for (NSString * title in titles) {
        UIBarButtonItem * item = [self byEnergyNavItemWithTitle:title
                                                        isLeft:isLeft
                                                        target:target
                                                        action:action
                                                          tags:[tags[i++] integerValue]];
...
- (UIBarButtonItem *)byEnergyNavItemWithTitle:(NSString *)title
    ...
    CFButton * btn = [CFButton buttonWithType:UIButtonTypeCustom];
    btn.frame = CGRectMake(0, 0, 30, 30);
    [btn setTitle:title forState:UIControlStateNormal];
    [btn addTarget:target action:action forControlEvents:UIControlEventTouchUpInside];
    btn.titleLabel.font = ByEnergyRegularFont(16);
    ...
    [btn setTitleColor:[UIColor colorByEnergyWithBinaryString:@"#353535"] forState:UIControlStateNormal];
    btn.tag = tag;
    [btn setImage:IMAGEWITHNAME(imageName) forState:UIControlStateNormal];
    btn.alignment = BDButtonAlignmentHorizontalImageRight;
    [btn sizeToFit];
...
- (UITableView *)tableView {
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, SCREENHEIGHT) style:self.isGrouped?UITableViewStyleGrouped:UITableViewStylePlain];
        ...
        RAC(_tableView,showNoNet) = _RACObserve([BEnergyAppStorage sharedInstance], isNoNetContent);
    }
    return _tableView;
}
...
- (UICollectionView *)collectionView {
    if (_collectionView == nil) {
        ...
        RAC(_collectionView,showNoNet) = _RACObserve([BEnergyAppStorage sharedInstance], isNoNetContent);
    }

...
#import "NYCycleScrollViewFlowLayout.h"

@interface NYCycleScrollView()<UICollectionViewDataSource,UICollectionViewDelegate>{
    float _oldPoint;
    NSInteger _dragDirection;
}
@property (nonatomic,strong) UIImageView *backgroundImageView;
@property (nonatomic,strong) NYCycleScrollViewFlowLayout *flowLayout;
...
@property (nonatomic, copy) NYFocusSelectBlock selectFocusBlock;//点击回调block
@end

static NSString *const cellID = @"cellID";

@implementation NYCycleScrollView
...
{
    NYCycleScrollView *cycleScrollView = [[NYCycleScrollView alloc]initWithFrame:frame];
...
{
    NYCycleScrollView *cycleScrollView = [[NYCycleScrollView alloc]init];
    cycleScrollView.infiniteLoop = infiniteLoop;
    cycleScrollView.autoScroll = infiniteLoop;
    cycleScrollView.imgArr = imageGroups;
    return cycleScrollView;
...
-(void)configUIWithFocusDataArray:(NSArray *)imgArr {
    ...
    [self initCollectionViewPosition];
}
...
-(void)configUIWithFocusDataArray:(NSArray *)imgArr selectFocusBlock:(NYFocusSelectBlock)selectFocusBlock {
    ...
    self.imgArr = imgArr;
    self.selectFocusBlock = selectFocusBlock;
    [self.collectionView reloadData];
    [self initCollectionViewPosition];
}
...
{
    return (SCREENWIDTH - self.itemWidth)/2;
}
-(void)layoutSubviews
{
...
//    self.flowLayout.itemSize = CGSizeMake(_itemWidth, self.bounds.size.height);
    ...
    if (self.backgroundImageView) {
        [self.backgroundImageView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.equalTo(weakself.collectionView.mas_centerX).mas_offset(0);
            make.centerY.equalTo(weakself.collectionView.mas_centerY).mas_offset(0);
            make.size.mas_equalTo(CGSizeMake(weakself.itemWidth, self.bounds.size.height));
        }];
    }
...
//初始化item位置
- (void)initCollectionViewPosition {
...
{
    NYCycleScrollViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellID forIndexPath:indexPath];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.selectFocusBlock) {
        self.selectFocusBlock([self currentIndex] % self.imgArr.count);
    }
}
...
-(UICollectionView *)collectionView
{
    if(_collectionView == nil)
    {
        ...
        //注册cell
        [_collectionView registerClass:[NYCycleScrollViewCell class] forCellWithReuseIdentifier:cellID];
...
-(UIPageControl *)pageControl
{
    if(_pageControl == nil)
    {
        ...
        _pageControl.pageIndicatorTintColor = [UIColor colorByEnergyWithBinaryString:@"ffffff" Alpha:0.5];
        _pageControl.userInteractionEnabled = NO;
    }
    return _pageControl;
}
-(NYCycleScrollViewFlowLayout *)flowLayout
{
    if(_flowLayout == nil)
    {
        _flowLayout = [[NYCycleScrollViewFlowLayout alloc]init];

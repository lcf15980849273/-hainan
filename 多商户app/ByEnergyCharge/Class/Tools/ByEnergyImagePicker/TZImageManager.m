...
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(TZAlbumModel *))completion{
    ...
    if (iOS8Later) {
        ...
        model = [self modelWithResult:fetchResult name:cameraRoll.localizedTitle];
        if (completion) completion(model);
...
- (BOOL)isCameraRollAlbum:(NSString *)albumName {
    ...
    if (version >= 800 && version <= 802) {
        ...
        return [albumName isEqualToString:@"Camera Roll"] || [albumName isEqualToString:@"相机胶卷"] || [albumName isEqualToString:@"所有照片"] || [albumName isEqualToString:@"All Photos"] || [albumName isEqualToString:@"最近添加"];
    }
}

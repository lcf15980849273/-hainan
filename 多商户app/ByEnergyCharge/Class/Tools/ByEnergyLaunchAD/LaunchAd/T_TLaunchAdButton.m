...
@implementation T_TLaunchAdButton
...
- (instancetype)initWithSkipType:(SkipType)skipType{
    ...
    if (self) {
        ...
        _skipType = skipType;
        CGFloat y = IsIphoneXLater ? 44 : 20;
        //环形
        if(skipType == SkipTypeRoundTime || skipType ==SkipTypeRoundText || skipType == SkipTypeRoundProgressTime || skipType == SkipTypeRoundProgressText){
            self.frame = CGRectMake(T_T_ScreenW-55,y, 42, 42);
        }else{//方形
            self.frame = CGRectMake(T_T_ScreenW-80,y, 70, 35);
        }
        switch (skipType) {
            case SkipTypeNone:{
                self.hidden = YES;

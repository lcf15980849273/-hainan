...
typedef NS_ENUM(NSUInteger, MCachePolicy){
    /**只从网络获取数据，且数据不会缓存在本地*/
    MCachePolicyIgnoreCache = 0,
    /**只从缓存读数据，如果缓存没有数据，返回一个空*/
    MCachePolicyCacheOnly = 1,
    /**先从网络获取数据，同时会在本地缓存数据*/
    MCachePolicyNetworkOnly = 2,
    /**先从缓存读取数据，如果没有再从网络获取*/
    MCachePolicyCacheElseNetwork = 3,
    /**先从网络获取数据，如果没有在从缓存获取，此处的没有可以理解为访问网络失败，再从缓存读取*/
    MCachePolicyNetworkElseCache = 4,
    /**先从缓存读取数据，然后在从网络获取并且缓存，在这种情况下，Block将产生两次调用*/
    MCachePolicyCacheThenNetwork = 5
};

/**请求方式*/
typedef NS_ENUM(NSUInteger, MRequestMethod){
    /**GET请求方式*/
    MRequestMethodGET = 0,
    /**POST请求方式*/
    MRequestMethodPOST,
...
typedef NS_ENUM(NSUInteger, MNetworkStatusType){
    /**未知网络*/
    MNetworkStatusUnknown,
    /**无网路*/
    MNetworkStatusNotReachable,
    /**手机网络*/
    MNetworkStatusReachableWWAN,
    /**WiFi网络*/
    MNetworkStatusReachableWiFi
};

typedef NS_ENUM(NSUInteger, MRequestSerializer){
    /**设置请求数据为JSON格式*/
    MRequestSerializerJSON,
    /**设置请求数据为二进制格式*/
    MRequestSerializerHTTP
};

typedef NS_ENUM(NSUInteger, MResponseSerializer) {
    /**设置响应数据为JSON格式*/
    MResponsetSerializerJSON,
    /**设置响应数据为二进制格式*/
    MResponseSerializerHTTP
};

/**请求的Block*/
typedef void(^MHttpRequest)(id _Nullable responseObject, BOOL isCache, NSError *error);

/**下载的Block*/
typedef void(^MHttpDownload)(NSString * _Nullable path, NSError * _Nullable error);

/*上传或者下载的进度*/
typedef void(^MHttpProgress)(NSProgress *progress);

/**网络状态Block*/
typedef void(^MNetworkStatus)(MNetworkStatusType status);
...
+ (void)uploadFileWithURL:(NSString *)url
               parameters:(NSDictionary *)parameters
                 ...
                 progress:(MHttpProgress)progress
                 callback:(MHttpRequest)callback;
...
+ (void)downloadWithURL:(NSString *)url
               ...
               progress:(MHttpProgress)progress
               callback:(MHttpDownload)callback;
...
/**
 ...
 @param requestSerializer MRequestSerializerJSON---JSON格式  MRequestSerializerHTTP--HTTP
 */
+ (void)setRequestSerializer:(MResponseSerializer)requestSerializer;
...
/**
 ...
 @param responseSerializer MResponseSerializerJSON---JSON格式  MResponseSerializerHTTP--HTTP

...
static NSString *const NetworkResponseCache = @"MNetworkResponseCache";
...
/*实时获取网络状态*/
+ (void)getNetworkStatusWithBlock:(MNetworkStatus)networkStatus{
        ...
        [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
            switch (status) {
                case AFNetworkReachabilityStatusUnknown:
                    networkStatus ? networkStatus(MNetworkStatusUnknown) : nil;
                    break;
                case AFNetworkReachabilityStatusNotReachable:
                    networkStatus ? networkStatus(MNetworkStatusNotReachable) : nil;
                    break;
                case AFNetworkReachabilityStatusReachableViaWWAN:
                    networkStatus ? networkStatus(MNetworkStatusReachableWWAN) : nil;
                    break;
                case AFNetworkReachabilityStatusReachableViaWiFi:
                    networkStatus ? networkStatus(MNetworkStatusReachableWiFi) : nil;
                    break;
                default:
                    break;
            }
...
#pragma mark -- 缓存描述文字
+ (NSString *)cachePolicyStr:(MCachePolicy)cachePolicy
{
    switch (cachePolicy) {
        case MCachePolicyIgnoreCache:
            return @"只从网络获取数据，且数据不会缓存在本地";
            break;
        case MCachePolicyCacheOnly:
            return @"只从缓存读数据，如果缓存没有数据，返回一个空";
            break;
        case MCachePolicyNetworkOnly:
            return @"先从网络获取数据，同时会在本地缓存数据";
            break;
        case MCachePolicyCacheElseNetwork:
            return @"先从缓存读取数据，如果没有再从网络获取";
            break;
        case MCachePolicyNetworkElseCache:
            return @"先从网络获取数据，如果没有再从缓存读取数据";
            break;
        case MCachePolicyCacheThenNetwork:
            return @"先从缓存读取数据，然后再从网络获取数据，Block将产生两次调用";
            break;
            
        default:
            return @"未知缓存策略，采用MCachePolicyIgnoreCache策略";
            break;
    }
}
...
+ (void)GETWithURL:(NSString *)url
       ...
       cachePolicy:(MCachePolicy)cachePolicy
          callback:(MHttpRequest)callback
{
    [self HTTPWithMethod:MRequestMethodGET url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)POSTWithURL:(NSString *)url
        ...
        cachePolicy:(MCachePolicy)cachePolicy
           callback:(MHttpRequest)callback
{
    [self HTTPWithMethod:MRequestMethodPOST url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)HEADWithURL:(NSString *)url
        ...
        cachePolicy:(MCachePolicy)cachePolicy
           callback:(MHttpRequest)callback
{
    [self HTTPWithMethod:MRequestMethodHEAD url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)PUTWithURL:(NSString *)url
       ...
       cachePolicy:(MCachePolicy)cachePolicy
          callback:(MHttpRequest)callback
{
    [self HTTPWithMethod:MRequestMethodPUT url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)PATCHWithURL:(NSString *)url
         ...
         cachePolicy:(MCachePolicy)cachePolicy
            callback:(MHttpRequest)callback
{
    [self HTTPWithMethod:MRequestMethodPATCH url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}
...
+ (void)DELETEWithURL:(NSString *)url
          ...
          cachePolicy:(MCachePolicy)cachePolicy
             callback:(MHttpRequest)callback
{
    [self HTTPWithMethod:MRequestMethodDELETE url:url parameters:parameters cachePolicy:cachePolicy callback:callback];
}


+ (void)HTTPWithMethod:(MRequestMethod)method
                   url:(NSString *)url
            parameters:(NSDictionary *)parameters
           cachePolicy:(MCachePolicy)cachePolicy
              callback:(MHttpRequest)callback
...
//        NSLog(@"\n请求参数 = %@\n请求URL = %@\n请求方式 = %@\n缓存策略 = %@\n版本缓存 = %@",parameters ? [self jsonToString:parameters]:@"空", url, [self getMethodStr:method], [self cachePolicyStr:cachePolicy], _cacheVersionEnabled? @"启用":@"未启用");
    ...
    if (cachePolicy == MCachePolicyIgnoreCache) {
    ...
    }else if (cachePolicy == MCachePolicyCacheElseNetwork){
        //先从缓存读取数据，如果没有再从网络获取
        [self httpCacheForURL:url parameters:parameters withBlock:^(id<NSCoding> object) {
            if (object) {
                NSError  *error = nil;
    ...
    }else if (cachePolicy == MCachePolicyNetworkElseCache){
    ...
    }else if (cachePolicy == MCachePolicyCacheThenNetwork){
        ...
        //缓存策略错误，将采取 MCachePolicyIgnoreCache 策略
        NSLog(@"缓存策略错误");
        [self httpWithMethod:method url:url parameters:parameters callback:callback];
    }
...
#pragma mark -- 网络请求处理
+(void)httpWithMethod:(MRequestMethod)method url:(NSString *)url parameters:(NSDictionary *)parameters callback:(MHttpRequest)callback{
    ...
    [self dataTaskWithHTTPMethod:method url:url parameters:parameters callback:^(NSURLSessionDataTask * _Nonnull task, id _Nullable responseObject) {
        if (_logEnabled) {
//            NSLog(@"请求接口 = %@\n请求结果 =%@",task.currentRequest.URL,responseObject);
        }
        [[self allSessionTask] removeObject:task];
        NSError  *error = nil;
        callback ? callback(responseObject, NO, error) : nil;
...
#pragma mark -- 上传文件
+ (void)uploadFileWithURL:(NSString *)url parameters:(NSDictionary *)parameters name:(NSString *)name filePath:(NSString *)filePath progress:(MHttpProgress)progress callback:(MHttpRequest)callback
{
...
#pragma mark -- 上传图片文件
+ (void)uploadImageURL:(NSString *)url parameters:(NSDictionary *)parameters images:(NSArray *)images name:(NSString *)name fileName:(NSString *)fileName mimeType:(NSString *)mimeType progress:(MHttpProgress)progress callback:(MHttpRequest)callback;
{
    NSURLSessionTask *sessionTask = [_sessionManager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //压缩-添加-上传图片
...
#pragma mark -- 下载文件
+(void)downloadWithURL:(NSString *)url fileDir:(NSString *)fileDir progress:(MHttpProgress)progress callback:(MHttpDownload)callback
{
...
+ (NSString *)getMethodStr:(MRequestMethod)method{
    switch (method) {
        ...
        case MRequestMethodPOST:
            return @"POST";
            break;
        case MRequestMethodHEAD:
            return @"HEAD";
            break;
        case MRequestMethodPUT:
            return @"PUT";
            break;
        case MRequestMethodPATCH:
            return @"PATCH";
            break;
...
+ (void)httpCacheForURL:(NSString *)url parameters:(NSDictionary *)parameters withBlock:(void(^)(id responseObject))block
{
    ...
    [_dataCache objectForKey:cacheKey withBlock:^(NSString * _Nonnull key, id<NSCoding>  _Nonnull object) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (_logEnabled) {
//                NSLog(@"缓存结果 = %@",[self jsonToString:object]);
            }
            block(object);
        });
    }];
...
+ (void)setRequestSerializer:(MRequestSerializer)requestSerializer{
    _sessionManager.requestSerializer = requestSerializer==MRequestSerializerHTTP ? [AFHTTPRequestSerializer serializer] : [AFJSONRequestSerializer serializer];
}

+ (void)setResponseSerializer:(MResponseSerializer)responseSerializer{
    _sessionManager.responseSerializer = responseSerializer==MResponseSerializerHTTP ? [AFHTTPResponseSerializer serializer] : [AFJSONResponseSerializer serializer];
}

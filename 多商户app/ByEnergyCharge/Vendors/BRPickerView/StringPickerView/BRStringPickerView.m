...
- (instancetype)initWithTitle:(NSString *)title
    ...
    if (self = [super init]) {
        ...
        if (defaultSelValue) {
            ...
            if ([defaultSelValue isKindOfClass:[NSString class]]) {
                self.selectedItem = [dataSource objectAtIndex:0];
            }
        }
...
- (void)loadData {
    ...
    if (self.isSingleColumn) {
        ...
        }else if ([_dataSource containsObject:self.selectedItem]) {
            [self.pickerView selectRow:[_dataSource indexOfObject:self.selectedItem] inComponent:0 animated:YES];
            [self pickerView:self.pickerView didSelectRow:[_dataSource indexOfObject:self.selectedItem] inComponent:0];
        }
    } else {
    ...
    if (!_pickerView) {
        ...
        if (self.isSingleColumn) {
            ...
            [self.selectedItems enumerateObjectsUsingBlock:^(NSString *selectedItem, NSUInteger component, BOOL *stop) {
                [weakSelf.dataSource[component] enumerateObjectsUsingBlock:^(id rowItem, NSUInteger rowIdx, BOOL *stop) {

...
//背景色
#define kBackgroundColor [UIColor colorWithRed:245.f green:245.f blue:245.f alpha:1]
...
- (void)setSubViewFrame{
    ...
    CGFloat emptyViewCenterY = originEmptyHeight * 0.5f;  //用来设置空列表时空页面的y值
...
- (void)setupActionBtn:(NSString *)btnTitle target:(id)target action:(SEL)action btnClickBlock:(LYActionTapBlock)btnClickBlock{
    ...
    self.actionButton.frame = CGRectMake(0, contentHeight + subViweMargin+_actionBtnVerticalMargin, btnWidth, btnHeight);
...
- (void)setq:(CGFloat)subViewMargin{
...
- (void)setActionBtnVerticalMargin:(CGFloat)actionBtnVerticalMargin{
    if (_actionBtnVerticalMargin != actionBtnVerticalMargin) {
        _actionBtnVerticalMargin = actionBtnVerticalMargin;
